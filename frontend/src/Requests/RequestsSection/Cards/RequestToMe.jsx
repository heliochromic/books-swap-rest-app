...
const RequestToMe = ({ request, refreshRequests }) => {
    const handleRequest = async (status) => {
        try {
            const response = await axios.put(`http://localhost:8000/api/requests/${request.requestID}`, {
                status: status,
            }, getConfig());
            console.log(response.data);
            refreshRequests(); // Оновлення запитів після зміни статусу
        } catch (error) {
        ...
        <div className="card">
            <div className="card-body swapContainer">
                ...
                <div className="request-buttons">
                    ...
                    <button onClick={() => handleRequest('R')} className="btn btn-outline-danger">Скасувати</button>
                </div>
            </div>
        </div>
    );
